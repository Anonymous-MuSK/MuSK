...
train_loader = RandomNodeSampler(data, num_parts=40, shuffle=True,
                                 num_workers=20)
test_loader = RandomNodeSampler(data, num_parts=5, num_workers=20)
...
model = DeeperGCN(hidden_channels=64, num_layers=nl).to(device)
...
max_acc = [0, 0, 0]
PATH = "./baseline_models/teacher" + str(nl) + '_' + str(ver) +  ".pth"
